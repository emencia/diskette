from diskette import settings as default_settings
...
def test_configuration_default_settings(settings):
    ...
    with the same value.
    """
    configuration_defaults = DisketteDefaultSettings()
    ...
    configuration_setting_names = [
        attr
        for attr in dir(DisketteDefaultSettings)
        if not attr.startswith("_") and attr.isupper()
    ]
    ...
    for item in configuration_setting_names:
        assert getattr(default_settings, item) == getattr(DisketteDefaultSettings, item)

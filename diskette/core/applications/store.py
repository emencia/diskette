...
class DjangoAppLookupStore:
    ...
    def check_invalid_labels(self, labels):
        """
        TODO
        """
        unknow_apps = []
    ...
    def get_models_inclusions(self, labels, excludes=None):
        """
        Returns labels to include in a dump.
        ...
        Returns:
            list: List of Datalookup nodes. This can not be a Datalookup dataset
            because we need to enforce sorting.
        """
        names = []
        ...
        for label in labels:
            ...
            # all models in their original registered order
                ...
                names.extend([
                    model
                    for model in self.registry.filter(label=label)[0].models
                ...
                if self.normalize_label(model_label, app=app_label) not in excludes:
                    names.append(
                        self.registry.filter_related(
                            "models", name__exact=model_label, app=app_label
                        )[0]
                    )
    ...
    def get_models_exclusions(self, labels, excludes=None):
        """
        Returns labels to exclude implicitely and explicitely from a dump.
        ...
        # Get involved label from resolved model nodes
        involved_apps = list(
            set([item.app for item in model_items])
        )

        # Base query only care about involved apps
        q = self.registry.filter(label__in=involved_apps).filter_related("models")

        # Finally ignore model for inclusion
